...
class MLP:
    def __init__(self, n_entrada, n_escondida, n_saida, taxa_aprendizado):
        ...
        self.v_anterior = np.random.uniform(-0.5, 0.5, (self.n_escondida, self.n_entrada + 1))
        self.w = np.random.uniform(-0.5, 0.5, (self.n_saida, self.n_escondida + 1)) 
        self.w_anterior = np.random.uniform(-0.5, 0.5, (self.n_saida, self.n_escondida + 1))
...
estatisticas = np.zeros((10, 2))
# print(rotulo[0])

melhor_acerto = 0
melhor_epoca = 0

for epoca in range(1, 10):
    mlp = MLP(120, 70, 26, 0.1);
    ...
    i = 1
    while ((not (np.array_equal(mlp.w, mlp.w_anterior) and np.array_equal(mlp.v, mlp.v_anterior))) and i < 1000*epoca): # Enquanto os pesos não convergirem
        mlp.v_anterior = mlp.v
        mlp.w_anterior = mlp.w
...
print("Melhor epoca:", melhor_epoca)